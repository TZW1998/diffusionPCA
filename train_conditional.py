...
lr = 2e-5
net = UNet_MLP(input_dim = 28*28, time_emb_dim = 512,num_classes = 10, ranks = 28, scale = 12, block_layer = 2).cuda()

optimizer = Adam(net.parameters(), lr = lr)

ema_decay = 0.995
ema_net = UNet_MLP(input_dim = 28*28, time_emb_dim = 512,num_classes = 10, ranks = 28, scale = 12, block_layer = 2).cuda()
...
for ep in range(epoch):
    for inp, out, rank_list, label_list in train_dataloader:
        steps += 1
        optimizer.zero_grad()
        est_out = net(inp.cuda(),rank_list.cuda(), label_list.cuda())
        ...
        if steps % 1000 == 0:
            torch.save(net.state_dict(), "conditional_model.pth")
            torch.save(ema_net.state_dict(), "conditional_ema_model.pth")
